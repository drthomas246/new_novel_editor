...
export default function CharacterEditPage() {
  ...
  const { character, setCharacter, chartData, saveCharacter } = useCharacter(id)
  ...
  return (
    <div className='max-w-xl mx-auto p-6 space-y-4 bg-white shadow rounded'>
      ...
      <Button className='bg-purple-600 hover:bg-purple-700 w-full' onClick={() => setShowModal(true)}>
        診断を実行
      </Button>

      <div className='flex justify-between'>
        <Button className='bg-blue-600 hover:bg-blue-700' onClick={saveCharacter}>
          保存
        </Button>
        <Button className='bg-gray-500 hover:bg-gray-600' onClick={() => router.push('/mypage/characters')}>
          キャンセル
        </Button>
      </div>
